...
bool CMainApplication::HandleInput()
{
	...
	vr::TrackingUniverseOrigin eOrigin = vr::TrackingUniverseStanding;
...
//		if (m_pHMD->GetControllerState(unDevice, &state, sizeof(state)))
		...
		if( m_pHMD->GetControllerStateWithPose( eOrigin, unDevice, &state, sizeof(state), &trackedDevicePose))
		{
			m_rbShowTrackedDevice[ unDevice ] = state.ulButtonPressed == 0;

			ProcessControllerStateAndPose(unDevice, state, trackedDevicePose);
		}
	}
...
void CMainApplication::RenderScene( vr::Hmd_Eye nEye )
{
	...
	SetPoseEyeProjectionMatrix(GetCurrentViewProjectionMatrix(nEye));

	if (nEye == vr::Eye_Left)
	{
		SetProjectionMatrix(m_mat4ProjectionLeft);
		SetPoseEyeMatrix(m_mat4eyePosLeft * m_mat4HMDPose);
	}
	else if (nEye == vr::Eye_Right)
	{
		SetProjectionMatrix(m_mat4ProjectionRight);
		SetPoseEyeMatrix(m_mat4eyePosRight * m_mat4HMDPose);
	}

	SetPoseMatrix(m_mat4HMDPose);
